...
Every estimator in KernelJax ultimately works with a {class}`~kerneljax.MixedData` sample. It records which columns are continuous, which are unordered categories, and which are ordered categories, and that declaration determines which kernel smooths each column.
...
Two features of it recur throughout the guide. First, its slope with respect to experience is
...
Second, $r$ does not appear on the right-hand side of $m(e,s,r)$. Holding experience and education fixed, $m(e, s, r_1) = m(e, s, r_2)$ for any two regions $r_1$ and $r_2$. That is the sense in which region is irrelevant to the conditional mean. No estimator in the guide is told either of these facts.
...
`MixedData` tells KernelJax how each covariate should be treated. Continuous variables are smoothed according to distance, ordered variables according to distance between their levels, and unordered variables according to whether their levels match.
...
Internally, columns are stored in block order, continuous first, then unordered, then ordered. Names, level counts, and bandwidths follow the same layout.
...
Estimators evaluate at their training points unless told otherwise. The `at` argument accepts another sample with the same column layout, and two helpers cover the most common ways of constructing one.

{func}`~kerneljax.grid` sweeps one column while holding the others at representative values. For a continuous column, the default `trim=0` produces $n$ equally spaced points between its smallest and largest observed values,
...
\min\left\{
s :
\sum_{\ell \le s}\hat p_{d\ell} \ge q
\right\}
&&&\text{ordered}
\end{aligned}
$$
...
{func}`~kerneljax.quantile_grid` builds a different path through the data. Rather than spacing one continuous variable evenly over its range, it evaluates the continuous and ordered columns along a shared ladder of probabilities while holding unordered columns at their modes.
...
\min\left\{
s :
\sum_{\ell \le s}\hat p_{d\ell} \ge p_j
\right\}, & d\text{ ordered}.
\end{cases}
$$
